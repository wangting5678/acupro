...
import {
  servicesByCategory, locations, staff, CONTACT,
  type Service,
} from "../data";
...
export function Booking() {
  ...
  const serviceLocations = useMemo(
    () => (service ? locationsForService(service.id) : []),
    [service],
  );
  const serviceStaff = useMemo(
    () => (service ? staffForService(service.id, locationId) : []),
    [service, locationId],
  );
  const slots = useMemo(
        ...
        <div className="container" style={{ maxWidth: 620 }}>
          <div className="card confirm-box">
            ...
            <p>
              ...
              <strong>{service?.title}</strong> on{" "}
              <strong>{date && fmtDate(date)} at {time}</strong>. Our team will
                  ...
                  <div className="svc-cat" key={cat.id}>
                    ...
                    <div className="pick">
                      {cat.items.map((s) => (
                        <button
                          key={s.id}
                          className={`pick-item ${service?.id === s.id ? "sel" : ""}`}
                          onClick={() => {
                            setService(s); setLocationId(null); setStaffId(null);
                            setDate(null); setTime(null); go(1);
                          }}
                        >
                          <span className="meta">
              ...
              <div>
                ...
                <div className="pick" style={{ marginBottom: 26 }}>
                  {(serviceLocations.length ? serviceLocations : locations.map((l) => l.id)).map((lid) => (
                    <button key={lid} className={`pick-item ${locationId === lid ? "sel" : ""}`}
                      onClick={() => { setLocationId(lid); setStaffId(null); }}>
                      <span className="name">{locName(lid)}</span>
                      <span>›</span>
                    </button>
                  ))}
                </div>
                ...
                <div style={{ display: "flex", gap: 12, marginTop: 26 }}>
                  ...
                  <button className="btn btn-primary" disabled={!locationId} onClick={() => go(2)}>
                    Continue →
                  </button>
                </div>
              </div>
                ...
                <div className="date-row">
                  {nextDays(14).map((d) => (
                    <button key={d.toISOString()}
                      className={`date-chip ${date?.toDateString() === d.toDateString() ? "sel" : ""}`}
                  ...
                  <>
                    ...
                    {slots.length ? (
                      <div className="slot-grid">
                        {slots.map((sl) => (
                          <div key={sl.time} className={`slot ${time === sl.time ? "sel" : ""}`}
